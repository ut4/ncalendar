define(['src/CalendarLayout', 'src/Modal', 'src/Toolbar', 'src/Header', 'src/ViewLayouts', 'src/Constants'], (CalendarLayout, Modal, Toolbar, Header, ViewLayouts, Constants) => {
    ...
    QUnit.module('CalendarLayoutComponent', () => {
        ...
        QUnit.test('renderöi kalenterin Constants.VIEW_DEFAULT-muodossa', assert => {
            ...
            assert.notEqual(ReactTestUtils.findRenderedComponentWithType(this.rendered, Modal.default), undefined);
            assert.notEqual(ReactTestUtils.findRenderedComponentWithType(this.rendered, Toolbar.default), undefined);
            assert.notEqual(ReactTestUtils.findRenderedComponentWithType(this.rendered, Header[expectedView]), undefined);
            const calendarLayout = ReactTestUtils.findRenderedComponentWithType(this.rendered, CalendarLayout.default);
        ...
        QUnit.test('Asettaa modalin propertyksi', assert => {
            const calendarLayout = ReactTestUtils.findRenderedComponentWithType(this.rendered, CalendarLayout.default);
            assert.ok(calendarLayout.modal instanceof Modal.default);
        });
